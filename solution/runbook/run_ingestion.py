"""
Run all bronze-layer ingestion jobs for assignment.
Loads:
  - raw_companies
  - raw_subscriptions
  - raw_support_tickets
Each table is written as a Delta table under jacobs.bronze.* with ingestion metadata.
"""
...
def load_ingest_module(filename):
    module_path = SOLUTION_DIR/"bronze"/filename
...
def run_all_ingestion():
    spark = get_spark("jacobs-bronze-ingestion")
    ingest_jobs = [
